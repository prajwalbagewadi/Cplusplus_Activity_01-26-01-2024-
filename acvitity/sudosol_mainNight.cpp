...
Topic=sudoku-solver-in-cplusplus without backtracking
...
bool checknum_for_row(int row,int col,int num)
{
    ...
    {
        if(col_i!=col && arr[row][col_i]!=0 && arr[row][col_i]==num)
        {
            return false;
        }
    }
...
bool checknum_for_col(int row,int col,int num)
{
    ...
    {
        if(row_i!=row && arr[row_i][col]!=0 && arr[row_i][col]==num)
        {
            return false;
        }
    }
...
bool checknum_for_grid(int gridStartRow,int gridStartcol,int num)
{
    for(int row=0;row<3;row++)
    {
        ...
        {
            if(arr[row+gridStartRow][col+gridStartcol]!=0 && arr[row+gridStartRow][col+gridStartcol]==num)
            {
                return false;
            }
        }
    ...
    for(int row=0;row<N;row++)
    {
        for(int col=0;col<N;col++)
        {
            ...
            */
            ...
            int result_numRow,result_numCol,result_numGrid;

            // if(num_canBePlacedIn_row == num_canBePlacedIn_col && num_canBePlacedIn_row == num_canBePlacedIn_grid)
            //if(num_canBePlacedIn_row != num_canBePlacedIn_col && num_canBePlacedIn_row != num_canBePlacedIn_grid)
            if(num_canBePlacedIn_row >= num_canBePlacedIn_col || num_canBePlacedIn_row <= num_canBePlacedIn_grid)
            {
            //cout<<num_canBePlacedIn_row<<"<=num_canBePlacedIn_row"<<endl;
                ...
                {
                    if(checknum_for_grid(startGridRow,startGridCol,num_canBePlacedIn_row)==true)
                    {
                        cout<<num_canBePlacedIn_row<<"<=num_canBePlacedIn_row_cond1"<<endl;
                        result_numRow=num_canBePlacedIn_row;
                        //arr[result.row][result.col]=result_numRow;
                        insert_into(result.row,result.col,result_numRow);
                        //cout<<"ansOf_Cond1 for_checknum_for_grid="<<num_canBePlacedIn_row<<endl;
                    }  
                    cout<<"ansOf_Cond1 for_checknum_for_row="<<num_canBePlacedIn_row<<endl;
                    //arr[result.row][result.col]=result_numRow; 
                    insert_into(result.row,result.col,result_numRow);
                }
                cout<<"ansOf_Cond1 for_checknum_for_row="<<num_canBePlacedIn_row<<endl; 
            ...
            //else if(num_canBePlacedIn_col == num_canBePlacedIn_row && num_canBePlacedIn_grid)
            //else if(num_canBePlacedIn_col != num_canBePlacedIn_row && num_canBePlacedIn_col != num_canBePlacedIn_grid)
            //if(num_canBePlacedIn_col != num_canBePlacedIn_row && num_canBePlacedIn_col != num_canBePlacedIn_grid)
            if(num_canBePlacedIn_col >= num_canBePlacedIn_row || num_canBePlacedIn_col <= num_canBePlacedIn_grid)
            {
                //cout<<num_canBePlacedIn_col<<"<=num_canBePlacedIn_col"<<endl;
                if(checknum_for_row(result.row,result.col,num_canBePlacedIn_col)==true)
                {
                    if(checknum_for_grid(startGridRow,startGridCol,num_canBePlacedIn_col)==true)
                    {
                        cout<<num_canBePlacedIn_col<<"<=num_canBePlacedIn_col_cond2"<<endl;
                        result_numCol=num_canBePlacedIn_col;
                        //arr[result.row][result.col]=result_numCol;
                        insert_into(result.row,result.col,result_numCol);
                    } 
                    cout<<"ansOf_Cond2 for_checknum_for_col="<<num_canBePlacedIn_col<<endl;
                    //arr[result.row][result.col]=result_numCol; 
                    insert_into(result.row,result.col,result_numCol);  
                }
                cout<<"ansOf_Cond2 for_checknum_for_col="<<num_canBePlacedIn_col<<endl;
            ...
            //else if(num_canBePlacedIn_grid != num_canBePlacedIn_row && num_canBePlacedIn_grid != num_canBePlacedIn_col)
            //if(num_canBePlacedIn_grid != num_canBePlacedIn_row && num_canBePlacedIn_grid != num_canBePlacedIn_col)
            if(num_canBePlacedIn_grid >= num_canBePlacedIn_row || num_canBePlacedIn_grid <= num_canBePlacedIn_col)
            {
                //cout<<num_canBePlacedIn_grid<<"<=num_canBePlacedIn_grid"<<endl;
                if(checknum_for_row(result.row,result.col,num_canBePlacedIn_grid)==true)
                {
                    if(checknum_for_col(result.row,result.col,num_canBePlacedIn_grid)==true)
                    {
                        cout<<num_canBePlacedIn_grid<<"<=num_canBePlacedIn_grid_cond3"<<endl;
                        result_numGrid=num_canBePlacedIn_grid;
                        //arr[result.row][result.col]=result_numGrid;
                        insert_into(result.row,result.col,result_numGrid);
                    }  
                    cout<<"ansOf_Cond3 for_checknum_for_grid="<<num_canBePlacedIn_grid<<endl;
                    //arr[result.row][result.col]=result_numGrid;  
                    insert_into(result.row,result.col,result_numGrid);
                }
                cout<<"ansOf_Cond3 for_checknum_for_grid="<<num_canBePlacedIn_grid<<endl;
